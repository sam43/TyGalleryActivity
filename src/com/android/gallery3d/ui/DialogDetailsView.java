...
public class DialogDetailsView implements DetailsViewContainer {
    ...
    private void setDetails(MediaDetails details) {
        ...
        String title = mActivity.getAndroidContext().getString(R.string.ty_details_title);
    ...
    private class DetailsAdapter extends BaseAdapter
        ...
        private void setDetails(Context context, MediaDetails details) {
            ...
            String path = null;
            String widthVaule = null;//TY liuyuchuan add for New Design Gallery
            for (Entry<Integer, Object> detail : details) {
                String value = null;
                String heightVaule = null;//TY liuyuchuan add for New Design Gallery
                switch (detail.getKey()) {
                    ...
                    case MediaDetails.INDEX_WIDTH:
                        ...
                        if (detail.getValue().toString().equalsIgnoreCase("0")) {
                            widthVaule = context.getString(R.string.unknown);
                            resolutionIsValid = false;
                        } else {
                            widthVaule = toLocalInteger(detail.getValue());
                        }
                        break;
                    case MediaDetails.INDEX_HEIGHT: {
                        mHeightIndex = mItems.size();
                        if (detail.getValue().toString().equalsIgnoreCase("0")) {
                            heightVaule = context.getString(R.string.unknown);
                            resolutionIsValid = false;
                        } else {
                            heightVaule = toLocalInteger(detail.getValue());
                        }
                        break;
                    }
                ...
                if (key == MediaDetails.INDEX_WIDTH){
                    continue;
                }else if(key == MediaDetails.INDEX_HEIGHT){
                    value = String.format("%s: %s %s %s %s", DetailsHelper.getDetailsName(
                            context, key), widthVaule, "*", heightVaule,
                            mActivity.getResources().getString(R.string.camera_pixel));
                }
                //TY liuyuchuan add end for New Design Gallery
                else if (details.hasUnit(key)) {
        ...
        public void onResolutionAvailable(int width, int height) {
            ...
            String widthString = String.format(mDefaultLocale, "%s: %d",
                    mActivity.getResources().getString(R.string.height), width);
            String heightString = String.format(mDefaultLocale, "%d",height);
            String widthHeight = widthString+"*"+heightString+
                    mActivity.getResources().getString(R.string.camera_pixel);
            mItems.set(mWidthIndex, String.valueOf(widthHeight));
