...
public class TyAlbumSetListSlidingWindow implements TyAlbumSetListDataLoader.DataListener{
    private static final String TAG = "Gallery2/TyAlbumSetListSlidingWindow";
    ...
    private final TyAlbumSetListDataLoader mSource;
    private int mSize;
    ...
    public TyAlbumSetListSlidingWindow(GalleryContext activity,
            TyAlbumSetListDataLoader source, int cacheSize) {
    ...
    public boolean dropNoCheck(int from, int to) {
		if(from == to){
			return false;
		}
		/*
		if(from < to){
			for(int i = from; i < to; i++){
				AlbumSetEntry temp = mData[(i+1) % mData.length];
		...
		*/
		return mSource.dropSlot(from, to);
    }
    ...
    private  void updateAlbumSetEntry(AlbumSetEntry entry, int itemIndex) {
        ...
        if(album!=null){
            if(album.getBucketId()== mDataManager.mCollectBucketId){
                 ...
                 entry.title = mContext.getResources().getString(R.string.ty_collect);
            }
        }
        entry.totalCount = totalCount;
    ...
    public void onContentChanged(int index) {	    
        ...
        updateAllImageRequests();
        //if (mListener != null && isActiveItem(index)) {
