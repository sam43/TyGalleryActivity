...
public class SelectionManager {
    ...
    public boolean inSelectAllMode() {
        return mInverseSelection ? mClickedSet.size() == 0 : mInverseSelection;
    }
    ...
    public int getTotalCount() {
        ...
        if(!mIsAlbumSet&&mSourceMediaSet.getBucketId()==mDataManager.mCollectBucketId){
           mTotal = mDataManager.getCollectCount();
        }
    ...
    private static boolean expandMediaSet(ArrayList<Path> items, MediaSet set, int maxSelection) {
        ...
        //TY liuyuchuan add begin for PROD103709484
        if(set.getBucketId() == mDataManager.mCollectBucketId){
            ArrayList<MediaItem> list = mDataManager.getCollects();
            for (MediaItem item : list) {
                items.add(item.getPath());  
            }
        }
        //TY liuyuchuan add end for PROD103709484
        return true;
    }
    ...
    public ArrayList<Path> getSelected(boolean expandSet, int maxSelection) {
        ...
        if (mIsAlbumSet) {
            ...
            if (mInverseSelection) {
                ...
                int index = 0;                
                //TY WB034 20150202 add begin for tygallery 
                if(mSourceMediaSet.getBucketId()==mDataManager.mCollectBucketId){
                    int temp = Math.min(total - index, MediaSet.MEDIAITEM_BATCH_FETCH_COUNT);
                    ArrayList<MediaItem> list = mDataManager.getCollectItem(0,temp);              
                    for (MediaItem item : list) {
