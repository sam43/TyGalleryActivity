...
public class LocalMergeAlbum extends MediaSet implements ContentListener {
    ...
    //TY zhencc modify New Design Gallery
    ...
    //TY wb034 20150309 add begin for tygallery
    private DataManager mDataManager;
    //TY wb034 20150309 add end for tygallery
    public LocalMergeAlbum(GalleryApp galleryApp,
            Path path, Comparator<MediaItem> comparator, MediaSet[] sources, int bucketId) {
        super(path, INVALID_DATA_VERSION);
        //TY wb034 modify begin for tygallery
        //we need the datamanager to checkout if its items in shoucangjia
        mContext = galleryApp.getAndroidContext();
        mDataManager = galleryApp.getDataManager();
        //TY wb034 modify end for tygallery
        mComparator = comparator;
        mSources = sources;
        mBucketId = bucketId;
        for (MediaSet set : mSources) {
    ...
    //TY zhencc end New Design Gallery
    ...
    public synchronized ArrayList<MediaItem> getMediaItem(int start, int count) {
        ...
        if(head.get(markPos) == null)return null;
    ...
    public void delete() {
        //TY wb034 20150309 add begin for tygallery
       //when delete the album,should check its item whether in shoucang
        ArrayList<MediaItem> mediaItems = getMediaItem(0,getMediaItemCount());
        for(MediaItem item:mediaItems){
            Path path =item.getPath();
            if(mDataManager.isAleadyCollect(path)){
                mDataManager.Collect(false, path);
            }
        }
        //TY wb034 20150309 add end for tygallery
    	ArrayList<Integer> deleteBucketId = new ArrayList<Integer>();
        for (int i=0; i<mSources.length; i++) {
        	mSources[i].delete();
        }
        deleteBucketId.add(mSources[0].getBucketId());
        
        ArrayList<Integer> sharePreBucketId = GalleryUtils.getAlbumsBucketIdFromSharedPre(mContext);
        Log.d(TAG, "deleteBucketId:" + deleteBucketId + " >>> sharePreBucketId:" + sharePreBucketId);
        if(sharePreBucketId != null){
        	for(Integer bucketId : deleteBucketId){
        		sharePreBucketId.remove(bucketId);
        	}
        	GalleryUtils.clearAlbumsSharedPre(mContext);
        	GalleryUtils.saveAlbumsBucketIdToSharedPre(mContext, sharePreBucketId);
        }
    }
    //TY zhencc modify for New Design Gallery
