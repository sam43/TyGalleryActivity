...
public abstract class MediaSet extends MediaObject {
    ...
    //TY wb034 20140119 add begin for tygallery
    public  synchronized MediaItem[] getCoverMediaItem(int count) {
        ArrayList<MediaItem> items = getMediaItem(0, count);
        if (items.size() > 0){
            MediaItem[] covers =new  MediaItem[items.size()];
            for (int i = 0; i < items.size(); i++) {
               covers[i]=items.get(i);
         }
            return covers;
        }
        return null;
    }
  //TY wb034 20140119 add end for tygallery
    ...
    public int getTotalMediaItemCount(int count) {
        return count==0?getTotalMediaItemCount():count;
    }
    ...
    protected int enumerateTotalMediaItems(
        ...
        for (int i = 0; i < m; i++) {
           //TY wb034 20150213 modify begin for tygallery to escape NullPointException
            MediaSet subMediaSet = getSubMediaSet(i);
            if(subMediaSet!=null)
            start += subMediaSet.enumerateTotalMediaItems(
