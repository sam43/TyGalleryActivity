...
public class BucketHelper {
            ...
            while (cursor.moveToNext()) {
                if ((typeBits & (1 << cursor.getInt(INDEX_MEDIA_TYPE))) != 0) {
                    ...
                    /*BucketEntry entry = new BucketEntry(
                	    ...
                	    File data = new File(cursor.getString(INDEX_DATA)).getParentFile();
                    BucketEntry entry = new BucketEntry(
                            cursor.getInt(INDEX_BUCKET_ID),
                            cursor.getString(INDEX_BUCKET_NAME),
                            data);
    ...
    public static class BucketEntry {
        ...
        //TY zhencc add for New Design Gallery
        public BucketEntry(int id, String name, File data) {
            bucketId = id;
            bucketName = Utils.ensureNotNull(name);
            albumFilePath = data;
        }
    ...
    //TY yuxin add begin for New Design Gallery
    public static boolean isNoneMediaData(
            ContentResolver resolver, String bucketId, int type) {
        boolean isNone = true;
        
        if (isNone && (type & MediaObject.MEDIA_TYPE_IMAGE) != 0) {
            Cursor cursor = resolver.query(Images.Media.EXTERNAL_CONTENT_URI, 
                new String[]{ImageColumns._ID}, 
                ImageColumns.BUCKET_ID + " = ?", 
                new String[]{bucketId}, null);
            if (cursor == null) {
                Log.w(TAG, "isNoneMediaData MEDIA_TYPE_IMAGE cannot open local database");
                return true;
            }
            try {
                isNone = cursor.getCount() > 0 ? false : true;
            } finally {
                Utils.closeSilently(cursor);
            }
        }

        if (isNone && (type & MediaObject.MEDIA_TYPE_VIDEO) != 0) {
            Cursor cursor = resolver.query(Video.Media.EXTERNAL_CONTENT_URI, 
                ...
                new String[]{bucketId}, null);
            if (cursor == null) {
                Log.w(TAG, "isNoneMediaData MEDIA_TYPE_VIDEO cannot open local database" );
                return true;
            }
            try {
                isNone = cursor.getCount() > 0 ? false : true;
    ...
    //TY yuxin add end for New Design Gallery
}
