...
/*TIANYURD: wangzhg add for bt dialer begin */
import android.os.SystemProperties;
/*TIANYURD: wangzhg add for bt dialer end */
...
public class MoviePlayer implements
	...
	/*TIANYURD: wangzhg add for bt dialer begin */
	private static final String ACTION_BT_DIAL_PLAYSTATECHANGE= "com.android.videoplayer.playstatechange";
	/*TIANYURD: wangzhg add for bt dialer end */
    ...
    public MoviePlayer(View rootView, final MovieActivity movieActivity,
        ...
        if (savedInstance != null) { // this is a resumed activity
			...
			/*TIANYURD: wangzhg add for bt dialer begin */
			if (SystemProperties.getBoolean("ro.ty.btdialer.support", false)) {
   		        sendBroadcastforbtdialer(ACTION_BT_DIAL_PLAYSTATECHANGE, "state", "playing");
			}
   		    /*TIANYURD: wangzhg add for bt dialer end */			
        }
    ...
    private void pauseVideo() {
		...
		/*TIANYURD: wangzhg add for bt dialer begin */
		if (SystemProperties.getBoolean("ro.ty.btdialer.support", false)) {
		    sendBroadcastforbtdialer(ACTION_BT_DIAL_PLAYSTATECHANGE, "state", "pause");
		}
		/*TIANYURD: wangzhg add for bt dialer end */
    ...
    public void onCompletion(MediaPlayer mp) {
        ...
        if (mPlayerExt.getLoop()) {
			...
			/*TIANYURD: wangzhg add for bt dialer begin */
			if (SystemProperties.getBoolean("ro.ty.btdialer.support", false)) {
   		        sendBroadcastforbtdialer(ACTION_BT_DIAL_PLAYSTATECHANGE, "state", "stop");
			}
   		    /*TIANYURD: wangzhg add for bt dialer end */
    ...
    public void onRewind() {
        mRewindAndForwardListener.onRewind();
    }

    public void onForward() {
        mRewindAndForwardListener.onForward();
    }

    public void sendBroadcastforbtdialer(String action, String cmd, String content) {
        if (LOG) {
            Log.v(TAG, "sendbtdialBroadcast(" + action + "," + cmd + "," + content + ")");
        }
    ...
    private void getVideoInfo(MediaPlayer mp) {
        if (!MovieUtils.isLocalFile(mMovieItem.getUri(), mMovieItem.getMimeType())) {
            //taoxj add begin
            Boolean METADATA_ALL = false;
            Boolean BYPASS_METADATA_FILTER = false;
            Metadata data = null;
            try {
                  METADATA_ALL = (Boolean)MediaPlayer.class.getDeclaredField("METADATA_ALL").get(null);
                  BYPASS_METADATA_FILTER = (Boolean)MediaPlayer.class.getDeclaredField("BYPASS_METADATA_FILTER").get(null);
                  Method m = mp.getClass().getDeclaredMethod("getMetadata",Boolean.class,Boolean.class);
                  data =(Metadata)m.invoke(mp,new Boolean[]{METADATA_ALL},new Boolean[]{BYPASS_METADATA_FILTER});
            } catch (Exception e) {
                e.printStackTrace();
            }
            //taoxj add end
    ...
    private class ControllerRewindAndForwardExt implements IRewindAndForwardListener {
        ...
        public void onRewind() {
            ...
            if (mVideoView != null && mVideoView.canSeekBackward()) {
				...
				/*TIANYURD: wangzhg add for bt dialer begin */
				if (SystemProperties.getBoolean("ro.ty.btdialer.support", false)) {
   		            sendBroadcastforbtdialer(ACTION_BT_DIAL_PLAYSTATECHANGE, "state", "backwards");
				}
   		        /*TIANYURD: wangzhg add for bt dialer end */
        ...
        public void onForward() {
            ...
            if (mVideoView != null && mVideoView.canSeekForward()) {
				...
				/*TIANYURD: wangzhg add for bt dialer begin */
				if (SystemProperties.getBoolean("ro.ty.btdialer.support", false)) {
   		            sendBroadcastforbtdialer(ACTION_BT_DIAL_PLAYSTATECHANGE, "state", "speed");
				}
   		        /*TIANYURD: wangzhg add for bt dialer end */	
                mVideoView.seekTo(targetDuration);
            } else {
