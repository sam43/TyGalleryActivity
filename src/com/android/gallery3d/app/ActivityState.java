...
abstract public class ActivityState {
    ...
    protected GalleryContext mActivity;
    ...
    void initialize(GalleryContext activity, Bundle data) {
        mActivity = activity;
        mData = data;
    }
    ...
    private void setScreenFlags() {
        final Window win = ((Activity)mActivity.getAndroidContext()).getWindow();
    ...
    protected void transitionOnNextPause(Class<? extends ActivityState> outgoing,
        ...
        //}
        //yuxin modify end for  New Design Gallery
    }

    protected void performHapticFeedback(int feedbackConstant) {
        ((Activity)mActivity.getAndroidContext()).getWindow().getDecorView().performHapticFeedback(feedbackConstant,
                HapticFeedbackConstants.FLAG_IGNORE_VIEW_SETTING);
    }

    protected void onPause() {
        if (0 != (mFlags & FLAG_SCREEN_ON_WHEN_PLUGGED)) {
            ((Activity)mActivity.getAndroidContext()).unregisterReceiver(mPowerIntentReceiver);
        }
    ...
    protected MenuInflater getSupportMenuInflater() {
        return ((Activity)mActivity.getAndroidContext()).getMenuInflater();
    }
}
