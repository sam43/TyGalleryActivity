...
public class StateManager {
    ...
    public StateManager(GalleryContext activity) {
        mActivity = activity;
    }
    ...
    public void startState(Class<? extends ActivityState> klass,
        ...
        if (!mStack.isEmpty()) {
            ...
            if(mStack.size() == 1){
                ArrayList<StateManager> sms = mActivity.getOtherStateManagers(this);
                if (sms != null && sms.size() > 0){
                    for (StateManager sm : sms){
                        sm.getTopState().onPause();
                    }
                }
            }
            /*TIANYU: yuxin add begin for New Design Gallery*/
    ...
    public void finishTasks(){
        ...
        while (mStack.size() > 1) {
            activityState = mStack.pop().activityState;
            if (mIsResumed) {
