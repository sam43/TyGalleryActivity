...
public class PhotoDataAdapter implements PhotoPage.Model {
    ...
    private MyCollectListener myCollectListener = new MyCollectListener();
    ...
    public PhotoDataAdapter(GalleryContext activity, PhotoView view,
            MediaSet mediaSet, Path itemPath, int indexHint, int cameraIndex,
            boolean isPanorama, boolean isStaticCamera, String bucketId) {
        mGalleryContext = activity;
        /*TIANYU: wb034 add begin for New Design Gallery*/
        mDataManager = activity.getDataManager();
        mIsCollectFolder = bucketId.equals(String.valueOf(mDataManager.mCollectBucketId));
        /*TIANYU: wb034 add end for New Design Gallery*/       	
    ...
    public void resume() {
        ...
        mDataManager.addCollectListener(myCollectListener);
    ...
    public void pause() {
        mIsActive = false;
        mDataManager.removeCollectListener(myCollectListener);
    ...
    public void moveTo(int index) {
        ...
        MediaItem item = getItem(index);
        if(item == null)return;
        mPhotoView.setMimeType(item.getMimeType());
        if(item.getWidth() != 0 && item.getHeight() != 0){
            mPhotoView.setSize(item.getWidth(), item.getHeight());
        }else{
            BitmapFactory.Options options = new BitmapFactory.Options();
            options.inJustDecodeBounds = true;
            BitmapFactory.decodeFile(item.getFilePath(),options);
            mPhotoView.setSize(options.outWidth, options.outHeight);
        }
        //TY liuyuchuan add end for  New Design Gallery
    ...
    private void updateTileProvider(ImageEntry entry) {
        ...
        if (screenNail != null) {
            if (fullImage != null) {
                ...
                //TY liuyuchuan modify begin for  New Design Gallery
                int width = 0;
                int height = 0;
                if (mPhotoView.w <= 0 || mPhotoView.h <= 0 || mPhotoView.type.equals("video/mp4")
                    || mPhotoView.type.equals("video/3gpp")){
                ...
                //TY liuyuchuan modify end for  New Design Gallery
            }
        } else {
            mTileProvider.clear();
        }
    ...
    private class ScreenNailJob implements Job<ScreenNail> {
        ...
        public ScreenNail run(JobContext jc) {
            ...
            return bitmap == null ? null : new TiledScreenNail(bitmap, mTiledResources, mGalleryContext);
            /*TIANYU: yuxin modify end for New Design Gallery*/
        }
    }
    ...
    private class GetUpdateInfo implements Callable<UpdateInfo> {
        ...
        private boolean needContentReload() {
            ...
            //if the albumSet is not collectFile,when we uncollect the item,and move to the next,the photo may return to the original one
           // if(noNeedChange){noNeedChange = false;return false;}
        ...
        public UpdateInfo call() throws Exception {
            ...
            info.size = mSize;
            if(noNeedChange){noNeedChange = false;info.isUpdate = false;}
            return info;
        }
    }
    ...
    private class UpdateContent implements Callable<Void> {
        ...
        public Void call() throws Exception {
            ...
            updateImageCache();
            //TY liuyuchuan add begin for PROD103706049
            if(current != null){
                mPhotoView.setMimeType(current.getMimeType());
                if(current.getWidth() != 0 && current.getHeight() != 0){
                    mPhotoView.setSize(current.getWidth(), current.getHeight());
                }else{
                    BitmapFactory.Options options = new BitmapFactory.Options();
                    options.inJustDecodeBounds = true;
                    BitmapFactory.decodeFile(current.getFilePath(),options);
                    mPhotoView.setSize(options.outWidth, options.outHeight);
                }
            }
            //TY liuyuchuan add end for PROD103706049
            updateTileProvider();
            updateImageRequests();
    ...
    private class ReloadTask extends Thread {
        ...
        public void run() {
            while (mActive) {
                ...
                //TY WB034 20150310 add begin for tygallery
                if (!info.isUpdate &&!mIsCollectFolder) continue;
              //TY WB034 20150310 add end for tygallery
                updateLoading(true);
                ...
                /*TIANYU: wb034 add begin for New Design Gallery*/
                if(mIsCollectFolder){
                    info.items = mDataManager.getCollectItem(info.contentStart,info.contentEnd);
                }else{
                    info.items = mSource.getMediaItem(info.contentStart, info.contentEnd);//wb034               		
                }
                /*TIANYU: wb034 add end for New Design Gallery*/ 
    ...
    //TY wb034 20150310 add begin for tygallery
    class MyCollectListener implements DataManager.CollectListener{
        @Override
        public void onCollectChanged(int size, boolean isAdd) {
           //if(!mIsCollectFolder)noNeedChange = true;
           if(mIsCollectFolder&&mReloadTask!=null)mReloadTask.notifyDirty();
        }
    }
    //TY wb034 20150310 add end for tygallery
}
