 ...
 *
 */
public class TyDragSortListView extends ListView {
	
	private String TAG = "TyDragSortListView";
    
    private View mFloatView;
    private Point mFloatLoc = new Point();
    private Point mFloatDrawLoc = new Point();
    ...
    public TyDragSortListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs,
                    R.styleable.TyDragSortListView, 0, 0);

            mItemHeightCollapsed = Math.max(1, a.getDimensionPixelSize(
                    R.styleable.TyDragSortListView_collapsed_height, 1));

            mTrackDragSort = a.getBoolean(
                    R.styleable.TyDragSortListView_track_drag_sort, false);
            ...
            mFloatAlpha = a.getFloat(R.styleable.TyDragSortListView_float_alpha, mFloatAlpha);
            mCurrFloatAlpha = mFloatAlpha;

            mDragEnabled = a.getBoolean(R.styleable.TyDragSortListView_drag_enabled, mDragEnabled);

            mSlideRegionFrac = Math.max(0.0f,
                    Math.min(1.0f, 1.0f - a.getFloat(
                            R.styleable.TyDragSortListView_slide_shuffle_speed,
                            0.75f)));

            mAnimate = mSlideRegionFrac > 0.0f;

            float frac = a.getFloat(
                    R.styleable.TyDragSortListView_drag_scroll_start,
                    mDragUpScrollStartFrac);

            setDragScrollStart(frac);

            mMaxScrollSpeed = a.getFloat(
                    R.styleable.TyDragSortListView_max_drag_scroll_speed,
                    mMaxScrollSpeed);

            removeAnimDuration = a.getInt(
                    R.styleable.TyDragSortListView_remove_animation_duration,
                    removeAnimDuration);

            dropAnimDuration = a.getInt(
                    R.styleable.TyDragSortListView_drop_animation_duration,
                    dropAnimDuration);

            boolean useDefault = a.getBoolean(
                    R.styleable.TyDragSortListView_use_default_controller,
                    true);

            if (useDefault) {
                boolean removeEnabled = a.getBoolean(
                        R.styleable.TyDragSortListView_remove_enabled,
                        false);
                int removeMode = a.getInt(
                        R.styleable.TyDragSortListView_remove_mode,
                        TyDragSortController.FLING_REMOVE);
                boolean sortEnabled = a.getBoolean(
                        R.styleable.TyDragSortListView_sort_enabled,
                        true);
                int dragInitMode = a.getInt(
                        R.styleable.TyDragSortListView_drag_start_mode,
                        TyDragSortController.ON_DOWN);
                int dragHandleId = a.getResourceId(
                        R.styleable.TyDragSortListView_drag_handle_id,
                        0);
                int flingHandleId = a.getResourceId(
                        R.styleable.TyDragSortListView_fling_handle_id,
                        0);
                int clickRemoveId = a.getResourceId(
                        R.styleable.TyDragSortListView_click_remove_id,
                        0);
                int bgColor = a.getColor(
                        R.styleable.TyDragSortListView_float_background_color,
                        Color.BLACK);

                TyDragSortController controller = new TyDragSortController(
    ...
    private class AdapterWrapper extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            if (convertView != null) {
                View oldChild = ((TyDragSortItemView) convertView).getChildAt(0);
                mAdapter.getView(position, oldChild, parent);
            } else {
                View child = mAdapter.getView(position, null, parent);
                TyDragSortItemView v = new TyDragSortItemView(getContext());
            ...
            adjustItem(position + getHeaderViewsCount(), convertView, true);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mFloatView != null) {
            ...
            canvas.translate(mFloatDrawLoc.x, mFloatDrawLoc.y);
    ...
    private class LiftAnimator extends SmoothAnimator {
        ...
        public void onUpdate(float frac, float smoothFrac) {
            if (mDragState != DRAGGING) {
                ...
                mFloatDrawLoc.y = mY - mDragDeltaY;
    ...
    private class DropAnimator extends SmoothAnimator {
        ...
        public void onStart() {
            ...
            mInitDeltaY = mFloatDrawLoc.y - getTargetY();
        ...
        public void onUpdate(float frac, float smoothFrac) {
            ...
            final float deltaY = mFloatDrawLoc.y - targetY;
            ...
            if (f < Math.abs(deltaY / mInitDeltaY) || f < Math.abs(deltaX / mInitDeltaX)) {
                //mFloatLoc.y = targetY + (int) (mInitDeltaY * f);
            	   mFloatDrawLoc.y = targetY + (int) (mInitDeltaY * f);
                mFloatLoc.x = getPaddingLeft() + (int) (mInitDeltaX * f);
                mFloatDrawLoc.x = getPaddingLeft() + (int) (mInitDeltaX * f);
                doDragFloatView(true);
            }
        }
    ...
    private class RemoveAnimator extends SmoothAnimator {
        ...
        public void onUpdate(float frac, float smoothFrac) {
            ...
            if (mUseRemoveVelocity) {
                ...
                mFloatDrawLoc.x = (int) mFloatLocX;
    ...
    private void continueDrag(int x, int y) {
        ...
        mFloatDrawLoc.x = x - mDragDeltaX;
        mFloatDrawLoc.y = y - mDragDeltaY;
    ...
    private void adjustItem(int position, View v, boolean invalidChildHeight) {
        ...
        if (position == mFirstExpPos || position == mSecondExpPos) {
            if (position < mSrcPos) {
                ((TyDragSortItemView) v).setGravity(Gravity.BOTTOM);
            } else if (position > mSrcPos) {
                ((TyDragSortItemView) v).setGravity(Gravity.TOP);
            }
        }
    ...
    public boolean startDrag(int position, View floatView, int dragFlags, int deltaX, int deltaY) {
        ...
        mFloatDrawLoc.x = mX - mDragDeltaX;
        mFloatDrawLoc.y = mY - mDragDeltaY;
    ...
    private void updateFloatView() {
        ...
        if (mFloatViewManager != null) {
            mTouchLoc.set(mX, mY);
            mFloatViewManager.onDragFloatView(mFloatView, mFloatLoc, mFloatDrawLoc, mTouchLoc);
        }
     ...
     * your own is not passed, the default {@link TySimpleFloatViewManager}
    ...
    public interface FloatViewManager {
         ...
         * and the offset provided in {@link TyDragSortListView#startDrag}.
         * @param touch The current touch location (relative to DSLV
         * top-left).
         * @param pendingScroll 
         */
        public void onDragFloatView(View floatView, Point location, Point drawLocation, Point touch);
     ...
     * {@link TyDragSortListView.DragScrollProfile}.
     * 
     * @param ssp
     */
     ...
     * {@link TyDragSortListView#setDragScrollProfile(DragScrollProfile)} to
     * set custom profile.
     * 
     * @author heycosmo
     *
    ...
    private class DragScroller implements Runnable {
        ...
        public void stopScrolling(boolean now) {
            if (now) {
                TyDragSortListView.this.removeCallbacks(this);
                mScrolling = false;
            } else {
                mAbort = true;
            }
        ...
        public void run() {
            ...
            TyDragSortListView.this.layoutChildren();
            invalidate();
