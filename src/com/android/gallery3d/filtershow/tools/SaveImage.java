...
//TY zhencc 20150715 add for PROD103855686 begin
import android.graphics.BitmapFactory;
//TY zhencc 20150715 add for PROD103855686 end
...
public class SaveImage {
        ...
        if (mPreviewImage != null) {
            if (flatten) {
                ...
                if (putExifData(mDestinationFile, exif, mPreviewImage, quality)) {
                    ...
                    /*TIANYURD:yanghao modify for PROD102933262 start*/
                    /*
                    ...
                    /*TIANYURD:yanghao modify for PROD102933262 end*/
            ...
            Log.d(LOGTAG,"savedUri="+savedUri);
            if (mCallback != null && !noBitmap) { //TY zhencc 20150715 add "!noBitmap" for PROD103855686
            ...
            try {
                ...
                if (putExifData(mDestinationFile, exif, bitmap, quality)) {
                    ...
                    if (!flatten) {
                        ...
                        //TY zhencc 20150715 add for PROD103855686 begin
                        ContentValues value = getWidthAndHeightValues(mDestinationFile);
                        mContext.getContentResolver().update(savedUri, value, null, null);
                        uri = savedUri;
                        //TY zhencc 20150715 add for PROD103855686 end
                    }
                }
    ...
    //TY zhencc 20150715 add for PROD103855686 begin
    private ContentValues getWidthAndHeightValues(File file){
    	int w = getWidthFromPath(file.getAbsolutePath());
    	int h = getHeightFromPath(file.getAbsolutePath());
    	Log.d(LOGTAG,"getWidthAndHeightValues() w:"+w +", h:"+h);
        ContentValues values = new ContentValues();
        values.put(Images.Media.WIDTH, w);
        values.put(Images.Media.HEIGHT, h);
        return values;
    }

    private int getWidthFromPath(String path) {
        Bitmap bitmap = BitmapFactory.decodeFile(path);
        if (bitmap == null) return 0;
        return bitmap.getWidth();
    }
    
    private int getHeightFromPath(String path) {
        Bitmap bitmap = BitmapFactory.decodeFile(path);
        if (bitmap == null) return 0;
