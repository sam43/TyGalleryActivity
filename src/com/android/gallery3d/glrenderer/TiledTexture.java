...
public class TiledTexture implements Texture {
    ...
    /*TIANYU: yuxin delete begin for New Design Gallery*/
    ...
    /*TIANYU: yuxin delete end for New Design Gallery*/
    private static HashMap<String, Tile> sFreeTileHead = new HashMap<String, Tile>();
    private static final HashMap<String, Object> sFreeTileLock = new HashMap<String, Object>();
    ...
    public static class Uploader implements OnGLIdleListener {
        ...
        private final ArrayDeque<TiledTexture> mBgTexture =
                new ArrayDeque<TiledTexture>(INIT_CAPACITY);
        ...
        public synchronized void addBgTexture(TiledTexture t) {
            if (t.isReady()) return;
            mBgTexture.addLast(t);
        ...
        public boolean onGLIdle(GLCanvas canvas, boolean renderRequested) {
            ArrayDeque<TiledTexture> deque = mTextures.isEmpty() ? mBgTexture : mTextures;
            synchronized (this) {
                ...
                mIsQueued = !mTextures.isEmpty() || !mBgTexture.isEmpty();
    ...
    private static class Tile extends UploadedTexture {
        ...
        public String _glTag;
        /*TIANYU: yuxin add end for New Design Gallery*/

        public Tile(String glTag) {
            super(glTag);
            _glTag = glTag;
        }
        ...
        protected Bitmap onGetBitmap() {
            ...
            if (localBitmapRef != null && resources.sCanvas != null) {
                ...
                resources.sCanvas.drawBitmap(localBitmapRef, x, y, resources.sBitmapPaint);
                ...
                if (x > 0) resources.sCanvas.drawLine(x - 1, 0, x - 1, TILE_SIZE, resources.sPaint);
                if (y > 0) resources.sCanvas.drawLine(0, y - 1, TILE_SIZE, y - 1, resources.sPaint);
                if (r < CONTENT_SIZE) resources.sCanvas.drawLine(r, 0, r, TILE_SIZE, resources.sPaint);
                if (b < CONTENT_SIZE) resources.sCanvas.drawLine(0, b, TILE_SIZE, b, resources.sPaint);
                /*TIANYU: yuxin modify end for New Design Gallery*/
            }

            /*TIANYU: yuxin modify begin for New Design Gallery*/
            //return sUploadBitmap;
            return resources.sUploadBitmap;
            /*TIANYU: yuxin modify end for New Design Gallery*/
        }
    ...
    private static void freeTile(Tile tile) {
        ...
        tile.bitmap = null;
        Object tileLock = getFreeTileLock(tile._glTag);
        synchronized (tileLock) {
            tile.nextFreeTile = getFreeTileHead(tile._glTag);
            setFreeTileHead(tile._glTag, tile);
        }
    }

    private static Tile obtainTile(String glTag) {
        Object tileLock = getFreeTileLock(glTag);
        synchronized (tileLock) {
            Tile result = getFreeTileHead(glTag);
            if (result == null) return new Tile(glTag);
            setFreeTileHead(glTag, result.nextFreeTile);
    ...
    public TiledTexture(Bitmap bitmap, Resources resources, String glTag) {
        this(bitmap, resources, glTag, null);
    }

    public TiledTexture(Bitmap bitmap, Resources resources, String glTag, String reString) {
    ...
    private static Tile getFreeTileHead(String glTag) {
        return sFreeTileHead.get(glTag);
    }
    private static void setFreeTileHead(String glTag, Tile tile) {
        sFreeTileHead.put(glTag, tile);
    }
    
    private static Object getFreeTileLock(String glTag) {
        Object object = sFreeTileLock.get(glTag);
        if (object == null){
            object = new Object();
            sFreeTileLock.put(glTag, object);
        }
        return object;
    }
}
