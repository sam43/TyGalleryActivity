...
public abstract class BasicTexture implements Texture {
    ...
    private static WeakHashMap<String, WeakHashMap> sAllTextures
            = new WeakHashMap<String, WeakHashMap>();
    
    private static ThreadLocal sInFinalizer = new ThreadLocal();

    private static HashMap<String, String> sReserveMap
    ...
    //    }
    //}
    protected BasicTexture(GLCanvas canvas, int id, int state, String glTag) {
        setAssociatedCanvas(canvas);
        mId = id;
        mState = state;
        synchronized (sAllTextures) {
            WeakHashMap<BasicTexture, Object> weakHashMap = sAllTextures.get(glTag);
            if (weakHashMap == null){
                weakHashMap = new WeakHashMap<BasicTexture, Object>();
                sAllTextures.put(glTag, weakHashMap);
            }
            weakHashMap.put(this, null);
        }
    }

    protected BasicTexture(String glTag) {
        this(null, 0, STATE_UNLOADED, glTag);
    }
    ...
    //    }
    //}
    public static void yieldAllTextures(String glTag) {
        synchronized (sAllTextures) {
            WeakHashMap<BasicTexture, Object> weakHashMap = sAllTextures.get(glTag);
            if (weakHashMap != null){
                String reserveString = sReserveMap.get(glTag);
                for (BasicTexture t : weakHashMap.keySet()) {
                    String perReString = t.getPerReString();
                    if (reserveString == null 
                        ||perReString == null 
                        ||!TextUtils.equals(reserveString, perReString)){
                        t.yield();
                    }
                }
            }
            //sAllTextures.remove(glTag);
        }
    }
    ...
    //    }
    //}
    public static void invalidateAllTextures(String glTag) {
        synchronized (sAllTextures) {
            WeakHashMap<BasicTexture, Object> weakHashMap = sAllTextures.get(glTag);
            if (weakHashMap != null){
                for (BasicTexture t : weakHashMap.keySet()) {
    ...
    public static void setReserveString(String glTag, String reserveString){
        sReserveMap.put(glTag, reserveString);
    }
    
    public static void removeReserveString(String glTag){
        sReserveMap.remove(glTag);
    }

    public void setPerReString(String reString){
        mReString = reString;
    }
    
    public String getPerReString(){
        return mReString;
    }
}
