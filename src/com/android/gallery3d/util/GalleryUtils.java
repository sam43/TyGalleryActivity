...
//TY zhencc 20150724 add for PROD103841758 begin
...
import com.android.gallery3d.app.TyAlbumSetListDataLoader;//TY wb034 20150121 add for tygallery
import com.android.gallery3d.common.ApiHelper;
import com.android.gallery3d.data.DataManager;
import com.android.gallery3d.data.MediaItem;
import com.android.gallery3d.ui.TiledScreenNail;
import com.android.gallery3d.ui.TyAlbumSetListSlidingWindow;//TY wb034 20150121 add for tygallery
...
public class GalleryUtils {
    ...
    public static final String KEY_EX_TY_NO_CLOSESENCE = "ty-no-closescene";
    ...
    //TY zhencc add for New Design Gallery
    public static List<Integer> mEntriesBucketIdList = new ArrayList<Integer>();
    public static Map<Integer, String> mEntriesBucketMap = new HashMap<Integer, String>();
    public static List<File> mEntriesAlbumFilePathList = new ArrayList<File>();
    public static List<File> mEntriesAlbumFilePathListCopy = new ArrayList<File>();
    public static int mFrontAlbumCount = 0; 
    public static final String TY_DRAG_ALBUM = "TYDragAlbum";
    public static final String TY_DRAG_ALBUM_SIZE = "TYDragAlbum_size";
    public static final String TY_DRAG_ALBUM_KEY = "TYDragAlbum_key_";
    //TY zhencc end for New Design Gallery

    private static float sPixelDensity = -1f;
    private static boolean sCameraAvailableInitialized = false;
    private static boolean sCameraAvailable;
    //TY wb034 20150121 add for tygallery
...
/*TAINYURD:yanghao modify for PROD103218091 start*/
                        | Intent.FLAG_ACTIVITY_CLEAR_TASK
/*TAINYURD:yanghao modify for PROD103218091 end*/ 
                        );
        try {
...
/*TIANYURD:yanghao modify for PROD103353943 start*/
/*
        try {
...
/*TIANYURD:yanghao modify for PROD103353943 end*/		
    }
    ...
    public static String formatDuration(final Context context, int duration) {
        ...
        if (h == 0) {
            ...
            if(m == 0){
                durationValue = String.format(context.getString(R.string.details_s), s);
            }
    ...
    //TY zhencc add for for New Design Gallery
    public static boolean clearAlbumsSharedPre(Context context){
    	SharedPreferences prefs = context
    			.getSharedPreferences(TY_DRAG_ALBUM, Context.MODE_PRIVATE);
    	SharedPreferences.Editor editor = prefs.edit();
    	editor.clear();
    	return editor.commit();
    }
    
    public static boolean saveAlbumsToSharedPre(Context context, BucketEntry[] entries){
    	...
    	editor.putInt(TY_DRAG_ALBUM_SIZE, length);
    	
    	for(int i=0; i<length; i++){
    		editor.putInt(TY_DRAG_ALBUM_KEY + i, entries[i].bucketId);
    	}
    	
    	return editor.commit();
    }
    
    public static boolean saveAlbumsBucketIdToSharedPre(Context context,
    		ArrayList<Integer> albumsBucketId){
    	if(albumsBucketId == null || albumsBucketId.size() == 0){
    		return false;
    	}
    	...
    	SharedPreferences prefs = context
    			.getSharedPreferences(TY_DRAG_ALBUM, Context.MODE_PRIVATE);
    	SharedPreferences.Editor editor = prefs.edit();
    	int size = albumsBucketId.size();
    	editor.putInt(TY_DRAG_ALBUM_SIZE, size);
    	
    	for(int i=0; i<size; i++){
    		editor.putInt(TY_DRAG_ALBUM_KEY + i, albumsBucketId.get(i));
    	}
    	
    	return editor.commit();
    }
    
    public static ArrayList<Integer> getAlbumsBucketIdFromSharedPre(Context context){
    	SharedPreferences prefs = context
    			.getSharedPreferences(TY_DRAG_ALBUM, Context.MODE_PRIVATE);
    	
    	int size = prefs.getInt(TY_DRAG_ALBUM_SIZE, 0);
    	if(size == 0){
    		return null;
    	}
    	...
    	for(int i=0; i<size; i++){
    		albumsBucketId.add(prefs.getInt(TY_DRAG_ALBUM_KEY + i, 0));
    	}
    	
    	return albumsBucketId;
    }
    //TY zhencc end for New Design Gallery
    //TY wb034 20150113 add begin for tygallery
    private static String sSdPath =null;
    ...
    public static String getSDPath(Context context) {
        if (sSdPath == null){
            ...
            try {
                Method m = mStorageManager.getClass().getDeclaredMethod("getVolumeList");
                volumes = (StorageVolume[]) m.invoke(mStorageManager);
            } catch (Exception e) {
                e.printStackTrace();
            }
            //taoxj add end
            for (int i = 0; i < volumes.length; i++) {
                if (volumes[i].isRemovable() && volumes[i].allowMassStorage()) {
                    sSdPath = volumes[i].getPath();
                }
            }
        }
        return sSdPath;
    }
    public static int getExtraSourceBucketId(Context context,String path){
    	return getBucketId(getSDPath(context)+path);
    }
    public static int getPhoneSourceBucketId(Context context,String path){
    	return getBucketId(getInternalPath(context)+path);
    }
    public static String getInternalPath(Context context) {
        if (sInternalPath == null){
            ...
            for (int i = 0; i < volumes.length; i++) {
                if (!volumes[i].isRemovable() && volumes[i].allowMassStorage()) {
                    sInternalPath = volumes[i].getPath();
                }
            }
            if(sInternalPath == null){
               File sdFile = Environment.getExternalStorageDirectory();
               if(sdFile != null){
                  sInternalPath = sdFile.toString();
               }
            }
        }
  ...
  //TY wb034 20150113 add end for tygallery
}
